...
export default class TwitchTokensHelper {
    ...
    static async refreshToken() {
        let channelTokenData = await ItemStore.load(new SettingTwitchTokens(), 'Channel')
        ...
        let chatbotTokenData = await ItemStore.load(new SettingTwitchTokens(), 'Chatbot')
    ...
    private static async refresh(data: SettingTwitchTokens, key: string) {
        const clientData = await ItemStore.load(new SettingTwitchClient(), 'Main')
        ...
        if (!json.error && !(json.status >= 300)) {
            ...
            const success = await ItemStore.save(data, key)
