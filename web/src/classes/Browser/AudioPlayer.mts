...
export default class AudioPlayer {
    private _mainCallback: IAudioPlayedCallback = (nonce, status) => {
        console.log(`AudioPlayer: Played callback not set, ${nonce}->${status}`)
    } // Actually used but does not reference back through .call()
...
export class AudioPlayerInstance {
    ...
    private _callback: IAudioPlayedCallback = (nonce, status) => {
        console.log(`AudioPlayer: Played callback not set, ${nonce}->${status}`)
    } // Actually used but does not reference back through .call()
    ...
    private initAudio() {
        ...
        this._audio.addEventListener('error', (evt) => {
            doCallback(this, AudioPlayerState.ERROR)
        })
        this._audio.addEventListener('ended', (evt) => {
            doCallback(this, AudioPlayerState.OK)
        })
        this._audio.addEventListener('pause', (evt) => {
            doCallback.call(this, AudioPlayerState.ABORTED) // TODO: Appears to do false negatives
        })
